 ...
 *   { ok: true,  config: { target, start, end, help, rangeProvided } }
 ...
 * @returns {{ ok: true, config: object } | { ok: false, error: { code: string, message: string } }}
 */
export function parseArgs(raw) {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (a === '-h' || a === '--help') {
      ...
      if (!Number.isInteger(n) || n <= 0)
        return err('YEARS_VALUE', 'years must be positive integer');
      flags.years = n;
  ...
  if (flags.range && flags.years) {
    return err('CONFLICT_RANGE_YEARS', 'Provide either --range or -y/--years, not both.');
  }
  ...
  if (flags.positional[0] !== undefined) {
    ...
    if (!Number.isFinite(parsed) || !Number.isInteger(parsed) || parsed < 0) {
      return err(
        'INVALID_TARGET',
        `Invalid target sum: ${flags.positional[0]}. Provide a non-negative integer.`,
      );
    }
    target = parsed;
  }
  ...
  if (flags.years) {
    ...
    start = new Date(Date.UTC(currentYear - flags.years + 1, 0, 1));
    end = new Date(Date.UTC(currentYear, 11, 31));
  }

  const rangeProvided = Boolean(flags.range || flags.years);
  return { ok: true, config: { help: flags.help, target, start, end, rangeProvided } };
}
...
/** Enumeration of all parser error codes. */
